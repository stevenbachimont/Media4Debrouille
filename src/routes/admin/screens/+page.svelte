...
<div class="flex items-center justify-between">
	<div>
		...
		<p class="mt-1 text-sm text-slate-500">Statut des players (rafraîchi toutes les 10 s)</p>
	</div>
	<a
...
<div class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
	<table class="min-w-full divide-y divide-slate-200">
		...
		<tbody class="divide-y divide-slate-200">
			{#if screens.length === 0}
				<tr>
					<td colspan="9" class="px-4 py-8 text-center text-slate-500">Aucun écran. <a href="/admin/screens/new" class="text-slate-700 underline">En créer un</a>.</td>
				</tr>
			{:else}
				{#each screens as screen}
					...
					<tr class="hover:bg-slate-50">
						...
						<td class="px-4 py-3 text-right">
							<a href="/admin/screens/{screen.id}/activate" class="rounded p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900" title="Activer">🔗</a>
							<a href="/admin/screens/{screen.id}/edit" class="rounded p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900" title="Modifier">✏️</a>
							<button
								type="button"
